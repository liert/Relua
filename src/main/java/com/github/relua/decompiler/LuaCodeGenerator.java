...
public class LuaCodeGenerator {
    ...
    public String generate(Chunk chunk) {
        ...
        // 生成指令代码（使用基本块信息）
        generateBasicBlocksCode(registerManager, chunk, sb, 0);
     ...
     */
    private void generateBasicBlocksCode(RegisterManager registerManager, Chunk chunk, StringBuilder sb,
            int indentLevel) {
        List<InstructionHandler.BasicBlock> basicBlocks = instructionHandler.getBasicBlocks();
        if (basicBlocks.isEmpty()) {
            // 如果没有基本块信息，回退到原始的指令生成方式
            generateInstructions(chunk, sb);
            return;
        }

        // 按顺序生成每个基本块的代码
        for (int i = 0; i < basicBlocks.size(); i++) {
            registerManager.addRegister(i, new Register());
            InstructionHandler.BasicBlock block = basicBlocks.get(i);
            generateBasicBlockCode(registerManager, chunk, block, sb);
        }
    }
    ...
    private void generateBasicBlockCode(RegisterManager registerManager, Chunk chunk,
        ...
        // 检查基本块类型，生成相应的控制流语句
        if (block.isIfBlock()) {
            generateIfBlockCode(registerManager, chunk, block, sb, indentLevel);
        } else if (block.isLoopBlock()) {
            generateLoopBlockCode(registerManager, chunk, block, sb, indentLevel);
        } else {
            // 普通基本块，直接生成指令代码
            System.out.println("BasicBlock: " + block.getEndIndex());
            for (int i = block.getStartIndex(); i <= block.getEndIndex(); i++) {
                if (i < instructions.size()) {
                    generateIndentedInstruction(registerManager, chunk, instructions.get(i), i, sb, indentLevel);
                }
            }
        }
    }
    ...
    private void generateIfBlockCode(RegisterManager registerManager, Chunk chunk, InstructionHandler.BasicBlock block,
        ...
        if (!successors.isEmpty()) {
            ...
            for (int i = trueBlock.getStartIndex(); i <= trueBlock.getEndIndex(); i++) {
                if (i < instructions.size()) {
                    generateIndentedInstruction(registerManager, chunk, instructions.get(i), i, sb, indentLevel + 1);
                }
            }
        }
        ...
        if (successors.size() > 1) {
            ...
            if (falseBlock.isElseBlock()) {
                ...
                for (int i = falseBlock.getStartIndex(); i <= falseBlock.getEndIndex(); i++) {
                    if (i < instructions.size()) {
                        generateIndentedInstruction(registerManager, chunk, instructions.get(i), i, sb,
                                indentLevel + 1);
                    }
                }
            }
     ...
     * @param sb          字符串构建器
     */
    private void generateInstructions(Chunk chunk, StringBuilder sb) {
        List<Instruction> instructions = chunk.getInstructions();
    ...
    private String generateInstructionCode(RegisterManager registerManager, Chunk chunk, Instruction instruction,
        ...
        switch (opcode) {
            case MOVE:
                // 寄存器间数据移动
                return generateMoveCode(currentRegister, chunk, instruction, index);
            case LOADK:
                // 加载常量到寄存器
                return generateLoadKCode(currentRegister, chunk, instruction, index);
            case LOADBOOL:
                // 加载布尔值到寄存器
                return generateLoadBoolCode(currentRegister, chunk, instruction, index);
            case LOADNIL:
                // 加载nil到寄存器
                return generateLoadNilCode(currentRegister, chunk, instruction, index);
            case ADD:
            case SUB:
            case MUL:
            case DIV:
            ...
            case POW:
                // 算术运算
                return generateArithmeticCode(chunk, instruction, index);
            case UNM:
            case NOT:
            case LEN:
                // 一元运算
                return generateUnaryCode(chunk, instruction, index);
            case GETGLOBAL:
                // 获取全局变量
                return generateGetGlobalCode(currentRegister, chunk, instruction, index);
            case SETGLOBAL:
                // 设置全局变量
                return generateSetGlobalCode(currentRegister, chunk, instruction, index);
            case GETTABLE:
                // 获取表元素
                return generateGetTableCode(currentRegister, chunk, instruction, index);
            case SETTABLE:
                // 设置表元素
                return generateSetTableCode(chunk, instruction, index);
            case SELF:
                return generateSelfCode(currentRegister, chunk, instruction, index);
            case CALL:
                // 函数调用
                return generateCallCode(currentRegister, chunk, instruction, index);
            case RETURN:
                // 返回
                return generateReturnCode(chunk, instruction, index);
            case CONCAT:
                // 字符串连接
                return generateConcatCode(chunk, instruction, index);
            case JMP:
                // 跳转指令，根据上下文决定是否生成代码
                return generateJmpCode(currentRegister, chunk, instruction, index);
            case TEST:
            case TESTSET:
            case EQ:
            case LT:
            case LE:
                return generateIfCode(currentRegister, chunk, instruction, index);
            default:
                // 其他指令，暂时输出指令信息
     ...
     * @param sb               字符串构建器
     */
    private String generateReturnCode(Chunk chunk, Instruction instruction, int instructionIndex) {
        int a = instruction.getA();
        int b = instruction.getB();
